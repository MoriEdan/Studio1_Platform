...
public class PlayVLC : MonoBehaviour
{
  ...
  [Header("Use pre-installed VLC Player")]
  ...
  public string InstallPath = "C:\\Program Files (x86)\\VideoLAN\\VLC\\vlc.exe";
  ...
  [HideInInspector] public bool UseVlc210OrHigher = true;
  ...
  [HideInInspector] public Text Debtext;
  ...
  [HideInInspector] public bool UseGUIVideoPanelPosition;
  [HideInInspector] public RectTransform GuiVideoPanel;
  //[Header("Skip Video Hint")]
  //[Tooltip("Show a skip hint under the video.")]
  [HideInInspector] public bool ShowBottomSkipHint;
  [HideInInspector] public GameObject BottomSkipHint;
  [HideInInspector] private bool _focusInUpdate;
  ...
  private void CheckErrors()
  {
    ...
    if (this.VideoPath.Length < 5 && !this.PlayFromStreamingAssets)
      UnityEngine.Debug.LogError((object) "Please enter a valid video file name!");
    if (!this.VideoInBackground && this.LoopVideo && (this.CompleteFullscreen || !this.UseGUIVideoPanelPosition) && (!this.SkipVideoWithAnyKeyPress && !this.ShowBottomSkipHint) || this.UsedRenderMode == PlayVLC.RenderMode.FullScreenOverlayModePrimaryDisplay && !this.SkipVideoWithAnyKeyPress)
  ...
  public void Play()
  {
    ...
    this._vlc.StartInfo.FileName = !this.UseBuiltInVLC ? "C:\\Program Files (x86)\\VideoLAN\\VLC\\vlc.exe" : UnityEngine.Application.dataPath + "/StreamingAssets/vlc/vlc.exe";
    //UnityEngine.Debug.Log(UnityEngine.Application.dataPath + "/StreamingAssets/vlc/vlc.exe");
  ...
  private void Update()
  {
    ...
    if (!this.SkipVideoWithAnyKeyPress || !this._isPlaying || (Input.GetKeyDown(KeyCode.LeftAlt) || Input.GetKeyDown(KeyCode.RightAlt) || !Input.anyKeyDown) && !Input.GetKeyUp(KeyCode.Space))
      return;
    this.KillVLCProcess();
